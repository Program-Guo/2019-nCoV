...
var ChinaShow = function () {
    //以下代码为数据在线获取方式，已经通过此方式获取数据并保存为json文件，故注释掉这段代码并直接使用本地文件。
    //
    // //日期-Data ,其中Data为数组，里面放着{name:湖北,value:123}这样的键值对。
    // let dateDataMap = {};
    // //省份表
    ...
    myChart.showLoading();
    $.get('../nCov_data_analysis/province_total.json').done(function (dateDataMap) {
        let timeLineData = [];
        let options = [];
        for (const i in dateDataMap) {
            timeLineData.push(i)
            options.push({
                title: {
                    text: '新冠病毒疫情病毒(各省确诊病例截止2020.' + i + ')',
                    subtext: '数据来源--腾讯新闻',
                    sublink: 'https://news.qq.com//zt2020/page/feiyan.htm'
                },
                series: [{
